...
const backgroundCorrection = data => {
  data.forEach(px => {
    if (px[3] === 0 || px[1] === 0 || (px[0] === 1 && px[1] === 0 && px[2] === 0)) {
      px[0] = 0;
      px[1] = 0;
      px[2] = 0;
      px[3] = 0;
...
colorFilter.prototype.OUTLINE = (r, g, b) => {
  return (context, callback) => {
    ...
    const data = backgroundCorrection(img2array(imgData));
...
colorFilter.prototype.COLORLEVELS = layerColors => {
  return (context, callback) => {
    ...
    const newImage = context.createImageData(context.canvas.width, context.canvas.height);
    newImage.data.set(data.flat());
    context.putImageData(newImage, 0, 0);
    callback();
  };
